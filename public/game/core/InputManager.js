...
export default class InputManager {
  ...
  setLaneCount(count) {
    ...
    // Check if user has custom bindings in localStorage
    const saved = localStorage.getItem('rhythm-os-keybinds');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (parsed[count]) {
          this.keyMap = parsed[count];
          return;
        }
      } catch(e) {}
    }
  ...
  _onKeyDown(e) {
    if (e.repeat) return; // Don't re-trigger on hold for tap notes
    const lane = this.keyMap[e.code];
    if (lane === undefined) return;
    e.preventDefault();
    this._active.add(e.code);
    EventBus.emit('input:hit', {
      lane,
      hitTime: this.audio.currentTime
    });
  }
  ...
  getDefaultKeyMaps() {
    return JSON.parse(JSON.stringify(DEFAULT_KEY_MAPS));
  }
}
